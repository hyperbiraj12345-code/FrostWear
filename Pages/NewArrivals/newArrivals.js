...
// ===== RENDER PRODUCTS =====
const freshPicksGrid = document.getElementById('freshPicksGrid');

function updateWishlistButtons() {
    [freshPicksGrid, searchResultsGrid].forEach(grid => {
...
async function loadProducts() {
    try {
        const res = await fetch('newArrivals.json');
        allProducts = await res.json();

        renderProducts(allProducts, freshPicksGrid);
        updateCartBadge();
        updateWishlistBadge();
        updateWishlistButtons();
    } catch (err) {
        console.error('Failed to load new arrivals:', err);
        freshPicksGrid.innerHTML = '<p class="drawer-empty">Failed to load products.</p>';
    }
}
...
function renderProducts(products, container) {
    ...
    products.forEach(p => {
        ...
        let badgeHTML = '';
        if (p.badge) {
            const badgeClass = isSale ? 'product-badge sale-badge' : 'product-badge new-badge';
