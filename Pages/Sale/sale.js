...
async function loadProducts() {
    try {
        const res = await fetch('sale.json');
        allProducts = await res.json();
        renderProducts(allProducts, productsGrid);
    } catch (err) {
        productsGrid.innerHTML = '<p class="section-subtitle">Failed to load sale items. Please try again later.</p>';
    }
...
function performSearch() {
    ...
    document.getElementById('saleItems').scrollIntoView({ behavior: 'smooth' });
